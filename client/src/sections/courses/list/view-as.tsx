...
const ViewAs: FC<TypeOfViewProps> = ({ typeOfView, listOfCourses }: TypeOfViewProps) => {
  const { categories, selectedCategories, isActive, setIsActive, handleCategoryChange } =
    useFilterCourse();
      ...
      {typeOfView === 'grid' ? (
        <div className="grid grid-cols-3 ">
          {listOfCourses.map((course) => (
            <div key={course.id} className="m-3 drop-shadow-lg h-80 border-2 flex justify-between">
              <GridCard
                courseTitle={course.name}
                imageSource={'/image1.jpg'}
                category={'Category ' + String(course.categories)}
              ></GridCard>
            </div>
          ))}
        </div>
          ...
          {categories.map((category) => (
            <div key={category.id} className="pb-2 text-lg">
              <input
                type="checkbox"
                checked={
                  selectedCategories.find((cat) => cat.id === category.id)?.selected ?? false
                }
                onChange={() => {
